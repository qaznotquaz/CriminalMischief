...
public class Main {
    ...
    public static void main( String[] args ) throws InterruptedException, IOException {
        ...
        System.out.println("End of archived conversation.\nClosing in 10 seconds.");
        Thread.sleep(10000);
    }
    ...
    public static void actLine(JSONObject line) throws InterruptedException {
        ...
        switch (type){
            case dm:
                ...
                break;
            case header:
                System.out.println(otherHighlight(line.getString("text"), RESET));
            case diagnostic:
                break;
        }
    }
    ...
    //todo: label
    public static void displayDm(Script.MiniActor actor, JSONObject line) {
        String text = line.getString("text");
        int othersLen = 0;

        if(line.has("otherHighlights")){
            othersLen = otherHighlight(line.getString("otherHighlights"), actor.getColor()).length();
        }
        ...
        if (actor.isSide()) {
            ...
            System.out.printf("%" + (formatWidth + 18 + othersLen) + "s\n", toPrint);
        } else {
            String toPrint = String.format(" %s%s%s > %s%s%s",
                    actor.getColor(), actor.getName(), RESET,
                    actor.getColor(), text, RESET);
